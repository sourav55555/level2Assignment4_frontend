...
export default function Restaurant({ cat }) {
  ...
  return (
    <div className='pt-16 text-white'>
        ...
        <div className='flex items-center justify-center gap-5 mt-12 uppercase'>
          {
            cat.map((item,index) => (
              <div  key={item} className='flex items-center gap-5 '>
                <Button
                  variant="ghost"
                  className={`uppercase tracking-wider ${item === activeTab? 'border-b-amber-400' : "border-transparent"} border-b `}
                  onClick={()=> setActiveTab(item)}
                >{item}</Button>
                {index + 1 !== cat.length &&
                  <MdOutlineRestaurant className='text-secondary' />
                }
              </div>
            ))
          }
